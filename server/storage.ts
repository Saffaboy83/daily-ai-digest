...
export interface SearchResult {
  date: string;
  dateLabel: string;
  section: string; // "Top Stories" | "AI Developments" | "Newsletters" | "World News" | etc.
  title: string;
  snippet: string;
  url?: string;
  category?: string;
...
export interface IStorage {
  getDigest(date: string): Promise<{ date: string; data: DigestData } | null>;
  saveDigest(date: string, data: DigestData): Promise<void>;
  listDigestDates(): Promise<string[]>;
  searchAllDigests(query: string): Promise<SearchResult[]>;
}

// In-memory storage with the seed data pre-loaded
export class MemStorage implements IStorage {
  private digests: Map<string, DigestData> = new Map();

  constructor() {
    // Seed with today's digest
    this.digests.set("2026-03-14", seedDigest);
  }

  async getDigest(date: string) {
    const data = this.digests.get(date);
    if (!data) return null;
    return { date, data };
  }

  async saveDigest(date: string, data: DigestData) {
    this.digests.set(date, data);
  }

  async listDigestDates() {
    return Array.from(this.digests.keys()).sort().reverse();
  }

  async searchAllDigests(query: string): Promise<SearchResult[]> {
    const q = query.toLowerCase();
    const results: SearchResult[] = [];

    for (const [date, data] of this.digests.entries()) {
      ...
      for (const stock of data.marketPulse || []) {
        if (matches(q, stock.ticker, stock.name, stock.change)) {
          results.push({
            ...
            snippet: `${stock.price} (${stock.change})`,
            url: stock.url,
          });
        }
      }
    ...
    // Sort by date descending, then by section
...
📎 Swipe through the carousel for a visual breakdown of all 6 developments.`,
      ...
      carouselSlides: [
        {
          slideNumber: 1,
          headline: "6 AI Developments You Can't Ignore This Week",
          body: "The AI race just entered a new chapter.",
          emoji: "🚀",
        },
        {
          slideNumber: 2,
          headline: "NVIDIA Vera Rubin Architecture",
          body: "384GB HBM4 · 336B transistors · 10x lower inference cost. GTC 2026 keynote: Monday 11am PT.",
          emoji: "💎",
        },
        {
          slideNumber: 3,
          headline: "Anthropic: $100M Claude Partner Network",
          body: "Accenture, Deloitte, Cognizant, Infosys as anchors. New Claude Certified Architect cert.",
          emoji: "🤝",
        },
        {
          slideNumber: 4,
          headline: "Meta's AI Model Delayed",
          body: "\"Avocado\" failed reasoning, coding, and writing tests. $65B budget ≠ guaranteed results.",
          emoji: "⚠️",
        },
        {
          slideNumber: 5,
          headline: "Cursor: $50B Valuation",
          body: "xAI aggressively poaching Cursor leads. Developer tools are the new AI battleground.",
          emoji: "⚡",
        },
        {
          slideNumber: 6,
          headline: "GPT-5.4 + Gemini 3.1 Pro",
          body: "Native agency in GPT-5.4. Gemini 3.1 scores 77.1% on ARC-AGI-2 (2x previous). The reasoning wars are here.",
          emoji: "🧠",
        },
        {
          slideNumber: 7,
          headline: "What This Means for You",
          body: "AI agents are accelerating. Developer tools are king. Enterprise adoption at 68%. The question isn't if — it's when.",
          emoji: "🎯",
        },
        {
          slideNumber: 8,
          headline: "Which one surprised you most?",
          body: "Follow for daily AI intelligence. Like & repost to share with your network.",
          emoji: "👇",
        },
      ],
    },
